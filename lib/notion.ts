...
export const queryNotionDatabase = React.cache(async (databaseId: string) => {
  ...
  return Promise.all(
    response.results.map(async (page: any) => {
      ...
      const publishedAt: Date = page?.created_time
        ? new Date(page.created_time)
        : new Date(0);
      const updateAt: Date = page?.last_edited_time
      ...
      const readingTimeResult = readingTime(
        await countWordsInNotionPage(page?.id)
      );
...
export const queryNotionDatabaseWithRelPosts = React.cache(
  async (databaseId: string, tagsInput: string) => {
    const tagsArray: string[] = tagsInput
      .split(",")
    ...
    const relevantPosts = posts.filter(
      (post) =>
        ...
        post.tags.some((tag) => tagsArray.includes(tag.toLowerCase()))
    );

    const topPosts = relevantPosts
      .map((post) => ({
        ...post,
        matchingTagsCount: Array.isArray(post.tags)
          ? post.tags.filter((tag) => tagsArray.includes(tag.toLowerCase()))
              .length
          : 0, // Check if tags is an array before filtering
      }))
...
export const incrementLikes = React.cache(async (pageId: string, currentLike : number) => {
  try {
    ...
    await notion.pages.update({
      ...
      properties: {
        Like: {
          number: currentLike + 1,
        },
      },
    });
  } catch (error) {
    console.error(`Failed to update views for page ${pageId}:`, error);
  }
})
